...
[P4 导航与安全反射调研设计](AI_PLAYER_RESEARCH_AND_P4_DESIGN_CN.md)。该文档当前是
实施前设计候选，不表示下列能力已经编码。
...
- [ ] 熔岩、坠落、溺水、窒息、弹射物反射；
- [ ] 食物/生命/装备安全策略；
- [ ] 原版伤害、护甲/附魔/吸收、药水/状态效果与属性 Buff 的真实玩家规则继承；
- [ ] 动态模组 `DamageType`/`MobEffect`、标准 NeoForge 事件链和未知类型保守策略；
- [ ] 安全抢占、恢复和循环检测；
- [ ] 负载降级。
...
- [ ] 卡住后在限定时间恢复；
- [ ] 无路可走时诚实返回 `NO_PATH`；
- [ ] 原版伤害、药水和状态效果对 bot 的最终身体结果与真人规则一致；
- [ ] 非 `minecraft` 命名空间伤害/效果通过标准接口生效且未知 ID 不被丢弃；
- [ ] API 完全断开时仍会避险。
