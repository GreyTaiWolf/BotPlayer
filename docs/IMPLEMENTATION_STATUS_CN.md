...
> physical-attempt handshake Contract 和 P5D-A0/A1/A2/A3/A4/A5 有界蓝图/施工工作包/candidate-site/survey-assessment/placeable-item/loaded-world survey 数据 Contract 仍待各自 Java 21 CI；P5 的通用生存能力和 P6 的通用
...
| P6-B0 | 跨边界 physical-attempt start 预算握手 | 已编码为 ADR-0037 pure Java Contract，尚未集成：server owner 的有界 `offer → exact ACK → settle → replay-stable grant` 绑定 server instance、owner、完整 safe receipt、attempt ID、nonce、client not-after 与 reservation-derived physical-start deadline；未 settle offer 可精确 release，settle 后 grant/terminal/disconnect/expiry 只 tombstone、不退款，periodic `expireDueAttempts()` 回收 active slot。client-local gate 只 handoff 一次，未来实际 Provider/HTTP start 必须紧邻原子 `tryClaimPhysicalStart()`，并重新复核 identity、binding epoch、session、两类 deadline 和单调 clock；重复、close/rebind、expiry、rollback 都 fail-closed。没有 C2S/S2C codec、authenticated session/tick authority、client queue、lifecycle/disconnect/reaper 调用、Provider/HTTP、usage/billing 或 Minecraft 接线；当前增量仍待 Java 21 CI 与真实客户端/Provider E2E |
