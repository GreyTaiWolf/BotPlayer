...
| P5B | 广泛原版容器/工作站、制作、生产和日常生活 | Build #362 已自动验证严格白名单容器、工作站边界、Bot 私有末影箱、受限 wheat/甘蔗收获、牛繁殖、单笔村民交易和牛奶解毒纵切；末影箱只承诺账本隔离与守恒取消，不承诺逐槽回滚或同方块跨 Bot 并行。当前分支的 strict `UseItem` 已在 `HEAD` 和 `completeUsingItem()` 前双重围栏；最后 Tick Tick-event 漂移/取消、Finish 取消、`PlayerTickEvent.Post` 取消与 deadline 精确回执优先级均有新增源码回归，但仍待 Java 21 CI/NeoForge GameTest。这不等于通用容器、任意配方/作物/交易或自动药物策略，真实客户端与专用服仍待验证 |
...
| P6-B0/B1 | 跨边界 physical-attempt start 预算握手 | ADR-0037 的 pure Java Contract 已有 server owner 的有界 `offer → exact ACK → settle → replay-stable grant` 与 client-local one-claim gate。ADR-0041 新增但尚未注册的 atomic S2C offer、C2S ACK、S2C grant codec：offer 同时携带已受限 dispatch 和完整 redacted identity，ACK/grant 只编码 identity，round-trip/diagnostic 回归覆盖 exact correlation 与不泄 owner/nonce/prompt。R1 还新增未接 lifecycle 的 owner-thread pure holder：只对 canonical dispatch 以固定 2048+256 conservative admission、按 `(owner,bot,agent)` scope 创建 bounded ledger，并将 exact receipt identity 与 ticket/gate 的非终态 exact lookup 关联；client controller 已只为 canonical R1 offer 本地登记 factory/gate，返回 ACK 前不调用 Provider，收到 exact grant 后只在一次性 claim 成功的直接边界调用一次 Provider。它们均尚未由 v3 registrar/lifecycle 调用；当前没有 authenticated sender/session/tick authority、production owner/reaper、真实 network ingress、usage/billing 或 Minecraft 接线，当前增量仍待 Java 21 CI 与真实客户端/Provider E2E |
