...
> physical-attempt handshake、P6-B1 mock payload-path GameTest 源和 P5D-A0/A1/A2/A3/A4/A4-R1/A5/A6/A7/A8/A9/A10/A11 有界蓝图/施工工作包/candidate-site/survey-assessment/placeable-item/default-state registry/loaded-world survey/spatial-lease/own-inventory-material observation/work-package-material demand/item-total projection/package-site target manifest/package-survey raw evidence pairing 数据 Contract 仍待各自 Java 21 CI；P5 的通用生存能力和 P6 的通用
...
`ActionEnvelope` 的取消会在 Action mailbox 的晚期 drain 前标记活动 strict-natural 使用；Mixin 的
`updateUsingItem` HEAD 对全部 active strict `UseItem` fail-closed，`completeUsingItem()` 前的复核/提交
...
after snapshot 不能作为 ACK；adapter 只尝试一次 exact native-menu reread 以冻结诊断，身份/读取失败
同样 fail-close，随后仍由既有 runtime cleanup 经原版 close 恢复 menu/cursor。纯 Java FSM 回归覆盖
前/后抛错、foreign/late observation 与 ACK/retry 拒绝；当前增量仍待 Java 21 CI、真实 `clicked()`
故障 GameTest、跨 Tick cleanup 和 Minecraft 客户端/专用服验证，不能等同跨 menu 通用事务或 P5 完成。
...
验证。另有受限 P5A handler 处理请求的工具、白名单精确主手物品和显式普通副手，均冻结为原版
46 槽 `InventoryMenu` 的 `WorldMenuTransaction`；其直接 GameTest 源仍待 Java 21 CI/NeoForge
GameTest。它们不是通用装备/副手入口、策略选择或盾牌格挡；盾牌继续在普通副手路径 fail-close，
且不计入基础装备。
...
| 自主生存/战斗 | 受限纵切已自动验证；总退出门未计数 | Build #362 已覆盖受限生产 DAG、保存围栏、白名单工作站（含 Bot 私有末影箱）与有限自卫。当前增量另有 world-menu click dispatch 异常的 pure FSM/adapter fail-closed 边界，以及请求工具、白名单精确主手物品、普通副手的 direct GameTest 源；这些真实原版 GameTest 均待 Java 21 CI。跨 menu 通用事务、通用 lifecycle continuation、泛化工具/副手策略、广泛战斗与完整生存能力仍未完成或未验证 |
...
| `/botplayer combat shield-hold <name>` | 已编码；待 Java 21 CI/NeoForge GameTest | 固定要求等级 2；仅已装备精确原版副手盾牌、无竞争 owner、原版 `InventoryMenu` 空 cursor 时提交固定 8 Tick hold/release；不是策略性格挡或通用副手入口 |
...
| P5A | 技能 FSM、首条生存闭环、最小原版世界容器驱动 | Build #362 已自动验证受限资源—制作—存放 DAG、保存围栏和两阶段重启；当前 M1a 已为 world-menu click dispatch exception 增加 pure FSM/adapter fail-closed 边界，E1a 已为已确认 native `InventoryMenu` PICKUP 前缀的取消补上受限原版 close 收口和守恒 GameTest 源码。受限 `EQUIP_BASIC_TOOL`、白名单 `EQUIP_EXACT_MAIN_HAND` 与显式 `EQUIP_REQUESTED_OFFHAND` handler 已注册，新增直接 GameTest 源经 lifecycle plan 覆盖三者的真实 46 槽 `InventoryMenu` / `WORLD_MENU_TRANSACTION` 成功端点、守恒，以及副手盾牌、绑定工具或绑定已有副手在 action 前 fail-close；本轮 M1a/E1a/装备测试均仍待 Java 21 CI、真实原版故障/取消 GameTest 与跨 Tick cleanup 验证。跨 menu 通用事务、任意装备策略与独立专用服仍未完成或未验证 |
...
| P5C | 运输、游戏进程和高级战斗 | Build #362 已自动验证旧有的、有限自卫会话授权的单次 `MELEE_ATTACK` 窄 bridge；当前分支已把该路由迁入单个 owner-thread `TechniqueLifecycleCoordinator` 的 Contract，并让 self-defense terminal 查询与取消都按完整 immutable `ActionEnvelope` 收口（同三元组的 foreign envelope 不能成为 child evidence 或安全回执）。另有 P5C-S1 管理员固定 route：只接受已装备、冻结的精确原版副手盾牌，以一条 generic `UseItem(OFF_HAND, RELEASE_AFTER_HOLD)` 固定持有 8 Tick 后释放；无目标、移动、换装、重试、反击或普通停止入口，并要求所有竞争 owner 静止、原版 `InventoryMenu` 与空 cursor。纯 Java 回归和“无库存漂移/主手盾拒绝”GameTest 源已加入，但上述增量仍待 Java 21 CI 与 NeoForge GameTest；它不证明真实受击格挡、耐久或斧破盾。目标选择、移动、装备策略、泛化 Technique 路由和其余 P5C 能力仍未实现 |
...
| P5D-A4-R1 | 显式材料声明 registry/default-state 核验 | 已编码为 ADR-0042 stateless server-thread adapter：只接受已 exact-cover 的 A4 declaration，以 native `BuiltInRegistries.ITEM` 查每个显式 item；lookup 后 canonical key 还必须精确相等，只有 non-air `BlockItem` 的 block default 完整 fingerprint 与 target 完全相等才返回 true，missing/non-block/air/alias/partial/non-default/codec failure 都拒绝。没有 `useOn`、world read/load/write、NBT、inventory/availability/material lease、site assessment、placement candidate、Technique、Action、Skill、lifecycle、AI 或红石接线。direct GameTest 源覆盖默认 stone/stairs 正例、各类拒绝、off-thread fence，以及 declaration quantity 和一个 world witness block 不变；当前仍待 Java 21 CI、NeoForge GameTest 和 Minecraft 实机验证 |
| P5D-A7 | 活动 Bot 自身背包材料可用性观察 | 已编码为 ADR-0043 stateless server-thread adapter 与 immutable result：先要求 authoritative thread、正 generation 且 `runtimeHandle.player()` 精确为传入 body；A4-R1 registry/default-state preflight 失败优先返回无 finding 的 `UNAVAILABLE_REGISTRY`。只有 active exact `InventoryMenu.class`、`stillValid`、46-slot shape 与空 cursor 才读取一次 41-slot snapshot；仅以 default-stack item/damage/components fingerprint 计 main/hotbar `0..35`，armor、副手、cursor、世界/末影箱/任意容器不计。permanent/temporary 相同 item 先聚合，结果只为 `AVAILABLE|SHORTAGE|UNAVAILABLE_MENU|UNAVAILABLE_REGISTRY`；unavailable 不伪造零库存或 fence。没有 world 或容器内容 read、材料移动/扣除/预留、site/lease/placement、Technique、Action、Skill、lifecycle、AI 或红石接线。direct GameTest 源覆盖足量无 mutation、聚合短缺、装备/组件排除、cursor/通过 `openMenu` 打开的 native `ChestMenu` 拒绝、registry 拒绝和 valid declaration off-thread fence；当前仍待 Java 21 CI、NeoForge GameTest 和 Minecraft 实机验证，P5D/P5 仍未实现 |
| P5D-A8 | 工作包显式材料需求 | 已编码为 ADR-0044 的 pure Java immutable Contract：只接受 complete `ConstructionWorkPlan` 中真实存在的完整 package key 与 `equals` 的 A4 evidence，逐个 exact package cell 重导 canonical `(itemId, materialClass)` requirements，total 严格等于该 package cell 数；public constructor 也会拒绝 forged、遗漏、跨 package 或跨 blueprint 的 requirement。它保留 permanent/temporary 分类，不将 A7 item-level availability 回填为 package allocation，也不读 Minecraft/world/container、不会 reservation、slot lock、移动、放置或接入 site/Technique/Action/Skill/lifecycle/AI。JUnit 源覆盖 canonical/immutable、跨类别同 item、65/256 cell 的实际分包、blueprint/key drift、forged requirements 与 DTO purity；仍待 Java 21 `clean build`/JUnit，P5D/P5 仍未实现 |
| P5D-A9 | 工作包材料可用性只读投影 | 已编码为 ADR-0045 的 private-constructor pure Java projection：只接受 A8 demand 与 `equals` 的 A7 source observation；usable source 时只将这一个 package 的 class-level demand 临时按 itemId 合并，与 source `availableCount` 产生 canonical package item finding。全 Blueprint `SHORTAGE` 时单 package 可 `OBSERVED_ITEM_TOTALS_SUFFICIENT`，但多个 sufficient projection 不可相加/并行，也不回填 class、slot/source 或 allocation；unavailable menu/registry 原样传递且 findings 为空。没有 Minecraft/world/container read、menu-fence 延长、reservation、slot lock、移动、placement、site/Technique/Action/Skill/lifecycle/AI 接线。JUnit 源覆盖 class aggregation、isolated package sufficient、actual package shortage、unavailable、evidence drift、65/256 partition、canonical/immutable 与 purity/private constructor；仍待 Java 21 `clean build`/JUnit，P5D/P5 仍未实现 |
| P5D-A10 | 工作包候选站点目标投影 | 已编码为 ADR-0046 的 private-constructor pure Java projection：只接受 binding 自己 plan 中真实存在的完整 package key，逐个 canonical package cell 经 `binding.targetPosition(key, offset)` 重导 exact candidate coordinate；同 Blueprint 的另一个合法 partition 即使复用 ordinal 也只取 binding 已冻结的 cell list。没有 survey/assessment/lease/material/A7/A8/A9 依赖，不读 Minecraft/world/container、不生成 stand/facing/click/useOn/placement candidate，不 reservation 或接 site readiness、Action/Technique/Skill/lifecycle/AI。JUnit 源覆盖 65/256 每包 cell/coordinate、alternative 16/49 partition、stale/foreign/missing key、canonical/immutable 与 purity/private constructor；仍待 Java 21 `clean build`/JUnit，P5D/P5 仍未实现 |
| P5D-A11 | 工作包站点调查证据投影 | 已编码为 ADR-0047 的 private-constructor pure Java projection：只接受 A10 target manifest 与 `equals` 的完整 A3/A5 survey binding，逐个 canonical package target 经 package-key+offset coordinate fence 重验并按 offset 配对原始 `UNKNOWN|EMPTY|OCCUPIED` observation；observed tick 只转发，不派生或输出 package-local assessment、freshness、ready、permit 或 progress。同 Blueprint 的另一个合法 partition 即使复用 ordinal，或 site/anchor/plan 任一 drift，均拒绝。没有 A6/A7/A8/A9、material/reservation、Minecraft/world/container、stand/facing/click/useOn/placement candidate、Action/Technique/Skill/lifecycle/AI 接线。JUnit 源覆盖 65/256、mixed raw state、alternative 16/49 partition、binding drift、forged/reordered pair、canonical/immutable 与 purity/private constructor；仍待 Java 21 `clean build`/JUnit，P5D/P5 仍未实现 |
| P5D-A5 | 已加载候选施工站点调查 | 已编码为 ADR-0038 narrow server-thread adapter，尚未集成：仅遍历 exact binding 的 canonical Blueprint cells（≤256），先验证 server thread + dimension，再依次检查 build height/`isLoaded`，只有已加载 target 调用 `getBlockState`；air→`EMPTY`，完整 registry/properties state→`OCCUPIED`，unloaded/out-of-height/codec failure→`UNKNOWN`。新增 direct GameTest 源覆盖真实 loaded `EMPTY` + 完整匹配 `OCCUPIED` state 到 data-only `ACCEPTED_CANDIDATE`、无 findings 和无 world write；它不把 candidate 提升为 freshness、accepted site 或施工许可。无 chunk load/ticket/world write、NBT/容器、site/material lease、placement candidate、Technique、Action、Skill、lifecycle、AI 或红石接线。当前增量仍待 Java 21 CI、NeoForge GameTest 和 Minecraft 实机验证 |
...
1. 把 P5 的受限自动化纵切扩展为跨 menu 通用事务、泛化工具/副手策略、任意配方/作物/交易和广泛战斗；
