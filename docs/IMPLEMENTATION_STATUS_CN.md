...
| 自主生存/战斗 | 受限纵切已自动验证；总退出门未计数 | Build #362 已覆盖受限生产 DAG、保存围栏、白名单工作站（含 Bot 私有末影箱）与有限自卫。当前增量另有 world-menu click dispatch 异常的 pure FSM/adapter fail-closed 边界，但真实原版故障 GameTest 仍待验证；跨 menu 通用事务、通用 lifecycle continuation、工具/副手、广泛战斗与完整生存能力仍未完成或未验证 |
