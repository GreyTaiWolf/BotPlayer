...
严格 `UseItem` 只在同一个版本精确的 Mixin 中使用两个 `require = 1` 注入点：

- `LivingEntity.updateUsingItem(Lnet/minecraft/world/item/ItemStack;)V` 的 `HEAD` 继续做
  消费前围栏；
- 同一方法中对 `LivingEntity.completeUsingItem()V` 调用的 `BEFORE` 注入在最终原版提交点
  再次复核。只有这次复核通过，活动 strict state 才单向进入 `ENTERED`；随后不得允许嵌套
  `updateUsingItem` 使用外层保存的 stale stack 再次消费。

两个点都只处理活动 `BotServerPlayer` 的精确 strict natural `UseItem`。hand、物品指纹、
原生 inventory menu/cursor/41 槽快照或批准效果漂移时，只走已有原版
`RELEASE_USE_ITEM`/`stopUsingItem()` 清理并取消当次调用；不得直接写库存、效果、方块或
Skill 状态。真人、非 strict 和非自然完成的使用保持原版路径。
...
- 性能：只对当前 bot generation 的有界活动 strict state 复核；不扫描世界、不异步、不新增
  常驻队列。
- 安全：未知 manager、线程错误、状态漂移、fence 不匹配和 cancellation ingress 失败都失败关闭；
  未提交的使用会停止，已提交的使用只接受 exact receipt。没有新增凭据、网络或世界写入面。
- 许可证：不新增依赖。
...
- 纯 Java：延迟取消的 verified success、failed/stale 保留、L0 pause/resume，以及 receipt 与
  deadline 同 tick 的优先级；
- NeoForge GameTest：最后 Tick `LivingEntityUseItemEvent.Tick` 的效果漂移与取消、Finish
  callback 取消、`PlayerTickEvent.Post` 取消，均核验牛奶/毒效果与 exact ActionOutcome；
