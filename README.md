...
> P6 会话协调器、客户端安全 terminal-observation/Error-cleanup/间接重入收口、ledger-first proposal review Contract、受限 Technique→Action permit Contract 与 P5D-A0 有界蓝图数据 Contract
...
> P5D-A0 另有纯 Java 的有界 `Blueprint`/content hash/计划方块需求 Contract；计划方块 ID 不是背包
> item ID，也没有材料预留、NBT、site、work package 或世界动作。仍没有 approved construction route、
> `GroundPlace`、材料授权或任何真实建筑/红石世界动作。
