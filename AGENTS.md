...
| 玩家技术动作、跳劈、走位、真实施工 | `technique/`、`building/blueprint/`（A0 蓝图 DTO）、`building/construction/`（A1 exact-cover work-package DAG）、`building/site/`（A2 candidate binding、A3 caller-supplied evidence 的 fail-closed assessment、A5 `minecraft/` server-thread loaded-world sampler）、`building/material/`（A4 full-state explicit item declaration；其 `minecraft/` A4-R1 只核验 explicit non-air `BlockItem` 的 default-state candidate）与现有 `action/`、`navigation/`、`safety/` | ADR-0017、ADR-0029、ADR-0030、ADR-0033、ADR-0035、ADR-0036、ADR-0038、ADR-0042、`PLAYER_TECHNIQUE_BUILDING_COMBAT_DESIGN_CN.md`；不得复制 Action/Skill runtime，也不得把蓝图/分包/site survey DTO、`ACCEPTED_CANDIDATE`、loaded snapshot、declared item quantity 或 default-state observation 当作世界动作、lease、库存、真实 placement 或许可入口 |
