...
| AI Provider、对话、智能体状态 | `ai/`（含 P6-A0 token-reservation ledger 与 P6-A1a physical-retry budget context）、当前实现状态列出的 AI 包；目标边界见架构 §8、§11 | ADR-0012、ADR-0020、ADR-0031、ADR-0032；不得把“保存 Key”、未接线账本或 context 写成“AI 已接通/已计费” |
| 玩家技术动作、跳劈、走位、真实施工 | `technique/`、`building/blueprint/`（A0 蓝图 DTO）、`building/construction/`（A1 exact-cover work-package DAG）、`building/site/`（A2 candidate anchor/bounds binding）与现有 `action/`、`navigation/`、`safety/` | ADR-0017、ADR-0029、ADR-0030、ADR-0033、`PLAYER_TECHNIQUE_BUILDING_COMBAT_DESIGN_CN.md`；不得复制 Action/Skill runtime，也不得把蓝图/分包/site DTO 当作世界动作入口 |
