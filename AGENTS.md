...
| 玩家技术动作、跳劈、走位、真实施工 | `technique/`、`building/blueprint/`（A0 蓝图 DTO）、`building/construction/`（A1 exact-cover work-package DAG）、`building/site/`（A2 candidate binding、A3 caller-supplied evidence 的 fail-closed assessment、A5 `minecraft/` server-thread loaded-world sampler、A10 只把 binding 中真实 package 投影为 exact cell/coordinate target manifest）、`building/material/`（A4 full-state explicit item declaration；其 `minecraft/` A4-R1 只核验 explicit non-air `BlockItem` 的 default-state candidate；A7 只在 server thread 读取活动精确 Bot body 的 empty native `InventoryMenu`，按默认 stack 指纹统计 main/hotbar 瞬时材料短缺；A8 只从 exact plan 的真实 work package 与 A4 evidence 重导 `(itemId, materialClass)` demand；A9 只将一个 A8 demand 与同 evidence 的 A7 snapshot 作 item-total projection）与现有 `action/`、`navigation/`、`safety/` | ADR-0017、ADR-0029、ADR-0030、ADR-0033、ADR-0035、ADR-0036、ADR-0038、ADR-0042～ADR-0046、`PLAYER_TECHNIQUE_BUILDING_COMBAT_DESIGN_CN.md`；不得复制 Action/Skill runtime，也不得把蓝图/分包/site survey DTO、`ACCEPTED_CANDIDATE`、loaded snapshot、declared item quantity、default-state observation、A7 availability、A8 demand、A9 projection 或 A10 target manifest 当作世界动作、lease、库存预留、真实 placement 或许可入口 |
